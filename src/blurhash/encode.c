...
static float *multiplyBasisFunction(int xComponent, int yComponent, int width, int height, uint8_t *rgb, size_t bytesPerRow) {
	...
	for(int y = 0; y < height; y++) {
		for(int x = 0; x < width; x++) {
			...
			r += basis * sRGBToLinear(rgb[COMPONENTS * x + 0 + y * bytesPerRow]);
			g += basis * sRGBToLinear(rgb[COMPONENTS * x + 1 + y * bytesPerRow]);
			b += basis * sRGBToLinear(rgb[COMPONENTS * x + 2 + y * bytesPerRow]);
		}
	}
